      ...
      if pap_mount(name_osnova,iii) >= 1 :
         ...
         if chek_drive(iii) == False:
            ...
            while True:
               ...
               sleep(2)
               os.system (f'screen -dmS "{name_osnova}" rclone mount {name_osnova}:{iii}-1.d /{name_osnova}/{iii}-1.d --max-duration 0 --no-checksum --no-modtime --fast-list --max-read-ahead=0K --vfs-read-chunk-size 10M --vfs-read-chunk-size-limit off --buffer-size off --timeout 0 --drive-keep-revision-forever --drive-stop-on-download-limit --daemon --drive-v2-download-min-size 256M --allow-non-empty --read-only')
